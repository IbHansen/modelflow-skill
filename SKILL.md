...
| Authoring a model in markdown or LaTeX with `Makemodel` or the `%%Makemymodel` / `%Makemymodel` Jupyter magic — `>` / `>>` equation lines, tags (`<estimator>`, `<ident>`, `<smpl>`, `<constraints>`, ...), LIST/DO templating, `replacements=`, reports; magic-line options (`shlex` quoting, namespace resolution of `input_df=`/`estimator=`), multi-cell `segment=` assembly, render switches, sibling magics (`latexflow`, `latexmodelgrab`, `graphviz`, `dataframe`) | `references/makemodel.md` |
...
| Translating a GAMS model (CGE, MCP/PATH) and its GDX data to Makemodel input — `modelgrabgdx` static import (`GdxStaticDataset`, `static_to_frame`, `sets_to_lists`, `pair_list`), GAMS `$`-conditions/SUM/ALIAS → `doable`/`sum`/lists, MCP pairings → `endo=`, closures/complementarities, the literate `>`-line file layout | `references/gams-to-makemodel.md` |
